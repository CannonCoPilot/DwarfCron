...
Cages are stored in an animal stockpile (unless they have been worn out somehow, in which case they are stored in a refuse stockpile). Dwarves will attempt to collect and store cages in stockpiles if the "Dwarves Gather Animals" order ( - **Standing orders** tab - **Hauling** tab) is enabled.

Empty cage storage can be controlled by toggling the "empty cages" option for animal stockpiles.

1. Building and using a cage
thumb|270px|right|Cages work well for humanoids as well.thumb|270px|right|Capturing that thing was the "easy" part.*Art by kruggsmash*
...
You can then either build them on a tile via  (this is needed for linking a lever to them, or assigning a tame animal to it) or simply keep them stockpiled so they can be used to load cage traps.

When building a cage, you can choose a cage that already has something inside. To precisely select which cage to use, you can expand the list of cages with .

1. Creature containment
To assign creatures to a cage, first build the cage in the location you want using . You can only assign creatures to a cage that is not part of a dungeon zone. Cages in a dungeon will have prisoners assigned automatically as part of the justice system. To assign a creature to the cage, first select the cage, and then press the "+" button on its status page. Use  and the mouse wheel to quickly scroll up and down the list of creatures, and  to assign them to the cage. Creatures currently assigned to the cage are listed at the top; the rest of the creatures are listed in order of arrival on the map (including any inaccessible creatures). Note that tame grazing creatures will starve if left in cages, though pet owners may feed their pets. Instead of caging grazers, assign them to a pasture that has plenty of tasty grass and/or cave moss.

There is no particular labor for releasing creatures from built cages.  Select the cage to examine it (it must first be "built," not just stored - use  to build a cage, and then to toggle the animal(s) currently inside (animals assigned to the cage will show up on the cage's page).  Any available dwarf will perform the job, so beware of pitting untamed or hostile creatures with a weak dwarf.
...
You can get details about the creatures which are in a built cage by pressing  and then . There, you can scroll the list of creatures and get details, for example, you can tell whether a creature is male or female, which is useful if you are preparing a breeding program.

Vermin can also be assigned to cages, to save space or animal traps. However, if you try to release them ( and then ), the dwarf will pickup an animal trap and put the poor creature back in a stock cage.  To explicitly release an animal, instead "pull" the animal somewhere else: either assign the animal to a pit (which could simply be a hillside outside your fortress), or a pasture.  This has the benefit of making it clear where the animal is being released.  However, keep in mind the dwarf will uncage the animal first and then lead it to the release, so large/dangerous animals may escape en route.  Consider using the dump command to move the cage before release, or assign the animal stockpile next to the pit/pasture.

Bees show up in the list of creatures that can be assigned to a cage, but the dwarves will not start the task, and it will not be shown in the obs list.
...
To selectively remove only weapons from caged prisoners, first claim the entire prisoner stockpile, then -- to hide everything including cages, buildings and prisoner equipment. Go in to your standard (non-dfhack) stock screen and browse the weapons section. Anything tagged with a blue **H** is a hidden prisoners' weapon, dump them. Once you have selectively removed only their weapons, use -- to unhide everything. This works with everything like shields and armor, not just weapons.
...
<!-- Marking refuse for dumping can be done by pressing  for an unbuilt cage ( for a built cage), then selecting the cage and pressing , then selecting the item and pressing  again, and finally pressing  to mark the item (rather than the cage) for dumping.  Or if there is a lot of refuse, you can use -- to mass-dump everything in the tile, and then un-mark the cage itself. -->
1. How to quickly empty out many cages
If you have many cage traps then you may have trouble emptying out cages quickly enough at times. See Mass pitting for suggestions on how to quickly recycle cages. Be advised that there are now reports of frequent escapes while using this method.
...
1. Bugs
- Multiple creatures can be assigned to the same cage with no penalty. It is possible to fit hundreds of puppies with dozens of dragons, and whatever other animals are on hand, in a single cage with no ill effects. 
...
- When bringing a cage to the Trade Depot in order to trade it, any untameable creatures inside the cage will be freed.  Tamed (and presumably also wild but tameable) animals can be safely traded. 
- Aquariums cannot be used to store large fish (they will drown), though vermin fish work just fine. 
