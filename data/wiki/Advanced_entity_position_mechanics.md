...
1. What is an entity? What is a position?
An entity is an organizational structure that can have relationships with other entities, usually known as a civilization or a "site government" in the game, but merchant guilds, religious organizations, necromancer towers, and bandit groups are also entities. An entity can have positions - in most cases, these are hardcoded and generated by the game, but as for civilizations and sites, the raw files can be customised.
...
1. Position levels (Site/Civ) and their interaction
...
Civ-level positions are in charge of the civilization as a whole, managing national trade, laws, and wars. These are, for example, the vanilla monarch, diplomat and general. (Note that the general doesn't do anything in either mode in the current version.)
...
1. Table of interaction between different position levels
In this table, the possible interactions between different position levels are summarized. 
The header row shows the positions defining the tokens.

The left column shows the position type that is referred to.
Example:  

**referred position-type
**LAND_HOLDER

| civilization
| <span style="color:green">(baron is) APPOINTED_BY:MONARCH

color coding: 
- <span style="color:green">Exists in vanilla
- <span style="color:blue">Possible with mods
- <span style="color:red">Not possible
- <span style="color:orange">possible to some extent.
- <span style="color:gray">not yet fully investigated.
...
| Civilization
| <span style="color:green">APPOINTED_BY  <span style="color:green">COMMANDER  <span style="color:blue">REPLACED_BY  <span style="color:green">SUCCESSION
| <span style="color:blue">APPOINTED_BY  <span style="color:gray">COMMANDER  <span style="color:red">REPLACED_BY  <span style="color:orange">SUCCESSION **(1)**
| <span style="color:green">APPOINTED_BY  <span style="color:gray">COMMANDER  <span style="color:red">REPLACED_BY  <span style="color:blue">SUCCESSION
| <span style="color:gray">APPOINTED_BY  <span style="color:gray">COMMANDER  <span style="color:gray">REPLACED_BY  <span style="color:gray">SUCCESSION

| SITE
| <span style="color:red">APPOINTED_BY  <span style="color:gray">COMMANDER  <span style="color:red">REPLACED_BY  <span style="color:orange">SUCCESSION **(2)**
| <span style="color:green">APPOINTED_BY  <span style="color:green">COMMANDER  <span style="color:green">REPLACED_BY  <span style="color:blue">SUCCESSION
| <span style="color:red">APPOINTED_BY **(3)**  <span style="color:gray">COMMANDER  <span style="color:red">REPLACED_BY  <span style="color:orange">SUCCESSION **(4)**
| <span style="color:red">APPOINTED_BY  <span style="color:gray">COMMANDER  <span style="color:gray">REPLACED_BY  <span style="color:red">SUCCESSION

| LAND_HOLDER
| <span style="color:blue">APPOINTED_BY  <span style="color:gray">COMMANDER  <span style="color:red">REPLACED_BY **(6)**  <span style="color:blue">SUCCESSION
| <span style="color:green">APPOINTED_BY  <span style="color:gray">COMMANDER  <span style="color:red">REPLACED_BY  <span style="color:orange">SUCCESSION **(5)**
| <span style="color:orange">APPOINTED_BY  <span style="color:gray">COMMANDER  <span style="color:green">REPLACED_BY **(7)**  <span style="color:orange">SUCCESSION **(8)**
| <span style="color:gray">APPOINTED_BY  <span style="color:gray">COMMANDER  <span style="color:gray">REPLACED_BY  <span style="color:gray">SUCCESSION

| <span style="color:gray">CONQUERED_SITE
| <span style="color:red">APPOINTED_BY <span style="color:gray"> COMMANDER  <span style="color:gray"> REPLACED_BY   <span style="color:orange"> SUCCESSION
| <span style="color:red">APPOINTED_BY <span style="color:gray"> COMMANDER  <span style="color:gray"> REPLACED_BY  <span style="color:red"> SUCCESSION
| <span style="color:gray">APPOINTED_BY <span style="color:gray"> COMMANDER  <span style="color:gray"> REPLACED_BY  <span style="color:gray"> SUCCESSION
| <span style="color:red">APPOINTED_BY <span style="color:gray"> COMMANDER  <span style="color:gray"> REPLACED_BY  <span style="color:gray"> SUCCESSION

Remarks:
1. Works only in world-gen, if a unit with that civ-position is present at the site.
1. Works only in world-gen, the site-position holding unit will then move to the capital.
1. Is completely ignored
1. Works only in world-gen.
1. Works only in world-gen.
1. Won't appear at all
1. This is necessary for the landholder chain to work properly, it doesn’t work otherwise.
1. This works only outside of the landholder chain, so when landholders are simply regarded as civ-level nobles. might also work in world-gen
1. 1. Other interactions
...
1. Civ-level nobles living at your site
...
If they have certain [RESPONSIBILITY]ies, they do the tasks that go with them.

1. Fortress mode, world-gen and their differences
...
1. Specific Tags and functions
1. [PRECEDENCE]
The first defined position with precedence of 1 counts as the ruler of the civ. See also: PRECEDENCE. 

If the precedence is omitted or has a negative value, the game sets 0 as precedence, which seems to have no effect whatsoever.

When a unit has multiple positions, the name of the position with the highest rank in precedence is shown behind the unit's name.
...
1. World-gen effects of the [RESPONSIBILITY] of available positions
...
1. Outpost Liaisons and Diplomats
...
1. Automatic spreading of assumed, non-singular positions
...
This may probably also work with elected and appointed positions, but that may be depending on the death of the current holders and this needs more testing. Assumption works within a day, anyway. 
...
1. [REPLACED_BY] and [REQUIRES_POPULATION]

The token REPLACED_BY:position means that once the replacing position reaches its required population-number, the 'to-be-replaced' position will disappear. This is defined in REQUIRES_POPULATION. These tags are closely related to each other and seem to only have a meaningful function if combined. Nobles with REQUIRES_POPULATION require the population to have a specific size. 
...
1. Effect of replacement (and on [LAND_HOLDER]s)
...
Even if the next position is available but not visible, replacement still takes place. If a position is replaced, it is completely gone - it cannot be appointed, succeeded, elected or assumed any longer.
The unit immediately loses its position.
...
The holder of a position marked with [CONQUERED_SITE] cannot appoint any other [SITE] positions.
...
1. Availability of (new) positions
...
A position that requires a certain population will become available, even if it doesn't have [APPOINTED_BY] or [ELECTED]. In that case, see 'automatic assignment' and 'assumption'

In fortress mode, in some cases 'succession' is evaluated immediately after a position becomes available. See: Succession.
1. Visibility of positions
...
Positions that have an [APPOINTED_BY:position] token require that position to exist, to be available and appointed. Any position that is APPOINTED_BY can be appointed if the appointer's position is filled in. 
For site-positions to be appointed, it is required that the appointer is present at that location.
...
1. Automatic appointment in world-gen and on the civ level
...
1. Manual appointments by players in fortress mode
...
| Responsibility [DELIVER_MESSAGES] || Ambusher, Observer and Social skills

| Responsibility [ACCOUNTING] || Record keeper

| Responsibility [ESPIONAGE] || Schemer

| Responsibility [HEALTH_MANAGEMENT] || Diagnostician

| Responsibility [MAKE_INTRODUCTIONS] || Social skills

| Responsibility [MAKE_PEACE_AGREEMENTS] || Social skills

| Responsibility [MAKE_TOPIC_AGREEMENTS] || Social skills

| Responsibility [MANAGE_PRODUCTION] || Organizer

| Responsibility [MEET_WORKERS] || Social skills

| Responsibility [RECEIVE_DIPLOMATS] || Social skills

| Responsibility [RELIGION] || Social skills

| Responsibility [TAME_EXOTICS] || Animal trainer

| Responsibility [TRADE] || Social skills and Appraiser

| [SQUAD]-token || Tactician and Leader
...
Using premature succession, a position that has [ELECTED], [APPOINTED_BY], or both, can be filled directly.
Notably, positions that combine [ELECTED] and [APPOINTED_BY] are normally impossible to fill through regular mechanics. Premature succession bypasses both election and appointment entirely, making this combination uniquely reachable through this mechanism.
...
1. The remaining chain then advances (A → B).
1. If C still has empty slots, the final step (B → C) may occur.
...
- The succeeding position is [REPLACED_BY] the succeedable position. In this case, replacement removes the unit from the source position first, leaving no holder available for the succession mechanism to use.rst, leaving no holder available for the succession mechanism to use.
...
- It has assumed another position, as this always leaves the previous position vacant.
...
A Landholder is a **special civ-level noble** who gets a certain piece of land to hold, when that land is elevated to a certain level, determined by the tag [LAND_HOLDER] and by the landholder triggers in the game's settings. In vanilla, these are the baron, the count and the duke. 
...
1. Functioning of the regular landholder chain
...
1. Impossibilities for succession by a site-position
...
1. Replacement by a generic civ-position
...
1. Landholder with fixed number
...
1. Military
1. Squad management
In the vanilla game, the Squad interface only becomes available after a militia commander has been appointed. Until then, the window displays:
...
- Everything regarding COMMANDING and army-structure
    - ** [http://www.bay12forums.com/smf/index.php?topic=165213.msg8335682#msg8335682
...
- If a certain threshold is reached that makes an elected position electable, and the triggering event that initiates this evaluation is an assumption, then the resulting assumption is also referred to in the messages as an election
