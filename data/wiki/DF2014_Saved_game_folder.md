...
1. Saving
At any time when you are playing Dwarf Fortress, you can manually save the game through the escape menu option. Simply hit  and then select Save Game. Doing so assumes you want to exit and it will take you back to the main menu. Dwarf Fortress only permits one save per world: there are no do-overs!
...
1. Save folder format

| 20px|link= save
&emsp;20px|link= current
&emsp;20px|link= region# 
&emsp;20px|link= region#-year-month-day
...
This can become confusing if you frequently savescum<abbr id="info" class="information" title="To reload the last saved game whenever the player character dies or an unfavorable outcome has been encountered.">‡</abbr>. There is no way to change the name of these folders inside the game, but it is safe to change them using the standard methods of your operating system if the game is not running.  However, **never** alter or delete the folder with the name of the game you're playing *while saving* from the game, or while that game is running!
...
1. When you want to reuse it, copy that region back to "data/save".  
