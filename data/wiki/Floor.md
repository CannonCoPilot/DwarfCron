...
thumb|192px|right|Stone, wood and metal floors.A **floor** is a fixed map tile that creatures can walk upon (as opposed to *movable* features like bridges and hatches). A floor can be:
1. natural, as found on the surface and in caverns
1. artificial, as created by mining
1. constructed
1. implied, by constructing walls on the z-level below
...
Removing floors can cause cave-ins if done <s>in</s>correctly. This often leads to fun if the floor happens to overhang the dining hall or other central area. 
...
1. 3×3 floor, supported in middle
