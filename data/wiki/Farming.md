...
thumb|200px|General farming flowchart.
...
Plants take time to grow, depending on their type. Once a plant is fully grown, a dwarf will harvest it; by default, any dwarf will do this. Harvesting plants is not affected by any skill, although it provides a small amount of grower experience, so it's a good idea to set only your planters to harvest, not just anyone. To do that, go to  Labor, then click Standing Orders, then click Other, then click "Everybody Harvests". This is useful only to train your planter faster; once they're skilled enough, everyone can be allowed to harvest again, so the haulers can take care of half the farming work.
...
- Fortress-wide seed cap counts seeds carried by traders  (Fixed in version 51.06: Merchants no longer bring seeds when at seed cap.)
- Fortress-wide seed cap removes seeds that have already been planted  (Fixed in version 51.06.)
- Conflict between seed caps can cause all seeds for a crop to disappear  (Fixed in version 51.06.)
